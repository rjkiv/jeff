...
fn process_fpctrl(obj: &mut ObjInfo) -> Result<()> {
    ...
    let split_start = found_funcs.first_key_value().unwrap().0;
    let split_end = found_funcs.last_key_value().map(|(k, v)| *k + *v).unwrap();
    obj.link_order.push(ObjUnit {
        name: String::from("xdk/LIBCMT/fpctrl.cpp"),
        autogenerated: false,
        order: None,
    });
    obj.add_split(
        ...
        split_start.address,
        ObjSplit {
            unit: String::from("xdk/LIBCMT/fpctrl.cpp"),
            end: split_end.address,
            align: None,
            ..Default::default()
        },
    )?;
