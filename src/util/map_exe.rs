    ...
    fn debug_print(&self) {
        for (unit_idx, symbols_for_unit) in &self.unit_symbols {
            ...
            for (sec_idx, symbol_idxs) in symbols_for_unit {
                let mut msg =
                    format!("\t{} ({:04}): ", self.sections[sec_idx.0].name, symbol_idxs.len());
...
pub fn apply_map_exe(result: ExeMapInfo, obj: &mut ObjInfo) -> Result<()> {
    ...
    // String = the fixed TU split name
    // BTreeMap u32 key = the SectionIndex for the ObjInfo
    // BTreeMap value = vector of (split start addr, split)s
    let mut deduced_obj_splits: HashMap<String, BTreeMap<u32, Vec<(u32, ObjSplit)>>> =
        HashMap::new();
        ...
        for (sec_idx, symbol_idxs) in symbols_by_section {
            ...
            match (addrs_for_this_section.first(), addrs_for_this_section.last()) {
                (Some(first), Some(last)) => {
                    ...
                    let sec = &result.sections[sec_idx.0];
                    let target_sec_name =
                        obj.sections.get(sec.index - 1).expect("where section").name.clone();
    ...
    for (tu_name, tu_section_splits) in &deduced_obj_splits {
        log::debug!("Adjusted TU name: {}", tu_name);
        for (section_index, section_splits) in tu_section_splits {
            let section_name = obj.sections.get(*section_index).unwrap().name.clone();
            for (split_start_addr, split) in section_splits {
