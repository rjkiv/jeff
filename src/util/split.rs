...
use crate::{
    ...
    obj::{
        section_kind_for_section, ObjInfo, ObjReloc, ObjRelocations, ObjSection, ObjSectionKind,
...
fn create_gap_splits(obj: &mut ObjInfo) -> Result<()> {
    ...
    for (addr, split) in new_splits {
        ...
        obj.link_order.push(ObjUnit { name: split_unit, autogenerated: true, order: None })
    }
...
fn split_pdata(obj: &mut ObjInfo) -> Result<()> {
    ...
    if let Some((idx, _)) = obj.sections.by_name(".pdata")? {
        if let Some(sec) = obj.sections.get_mut(idx) {
            sec.splits.clear();
        }
    }
...
pub fn split_obj(obj: &ObjInfo, module_name: Option<&str>) -> Result<Vec<ObjInfo>> {
    ...
    for unit in &obj.link_order {
        ...
        // this originally used ObjKind::Relocatable,
        // but ObjInfo's kind param is never read, sooo i just kinda...removed it
        let split_obj = ObjInfo::new(unit.name.clone(), vec![], vec![]);
